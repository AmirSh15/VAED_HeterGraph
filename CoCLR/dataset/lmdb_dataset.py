...
__all__ = [
    "UCF101LMDB_2CLIP",
    "UCF101Flow_LMDB_2CLIP",
    "UCF101_2STREAM_LMDB_2CLIP",
    "K400_LMDB_2CLIP",
    "K400_Flow_LMDB_2CLIP",
    "K400_2STREAM_LMDB_2CLIP",
    "UCF101LMDB",
    "UCF101Flow_LMDB",
    "HMDB51LMDB",
    "HMDB51Flow_LMDB",
...
lmdb_root = "/media/amir_shirian/Amir/Datasets/Sound Recognition/UCF101"
...
class UCF101LMDB_2CLIP(object):
    def __init__(
        ...
        db_path=os.path.join(lmdb_root, "RGB/ucf101_frame.lmdb"),
        transform=None,
        mode="val",
        num_frames=32,
        ds=1,
        ...
        if mode == "val":
            split_mode = "test"
        video_info = pd.read_csv(
            os.path.join(root, "%s_split%02d.csv" % (split_mode, which_split)),
            header=None,
        )
        ...
        self.get_video_id = dict(
            zip(
                [i.decode() for i in self.db_order],
        ...
        for idx, row in tqdm(
            video_info.iterrows(), total=len(video_info), disable=True
        ):
            vpath, vlen, _, _ = row
            if (
                vlen - self.num_frames // 2 * self.ds - 1 <= 0
            ):  # allow max padding = half video
                drop_idx.append(idx)
    ...
    def frame_sampler(self, total):
        if (self.mode == "test") or self.window:  # half overlap - 1
            if total - self.num_frames * self.ds <= 0:  # pad left, only sample once
                ...
                seq_idx = (
                    np.expand_dims(np.arange(self.num_frames) * self.ds, 0) + start
                )  # [test_sample, num_frames]
                seq_idx = seq_idx.flatten()  # seq_idx.flatten(0)
        else:  # train or val
            if total - self.num_frames * self.ds <= 0:  # pad left
                sequence = np.arange(self.num_frames) * self.ds + np.random.choice(
                    range(self.ds), 1
                )
                seq_idx = np.zeros_like(sequence)
                sequence = sequence[sequence < total]
    ...
    def __getitem__(self, index):
        ...
        with env.begin(write=False) as txn:
            raw = msgpack.loads(
                txn.get(self.get_video_id[vname].encode("ascii")), raw=True
            )
...
class HMDB51LMDB(UCF101LMDB):
    def __init__(
        self,
        root="%s/../process_data/data/hmdb51"
...
class UCF101Flow_LMDB_2CLIP(object):
    def __init__(
        ...
        vlen_list_ordered = sorted(
            list(zip([i.decode() for i in self.db_keys], self.vlen_list)),
            key=lambda x: x[0],
        )
        vlen_list_ordered = [i[-1] for i in vlen_list_ordered]
        video_info = video_info.merge(
            pd.DataFrame(zip(vname_list, vlen_list_ordered), columns=[3, 4]),
            left_on=3,
            right_on=3,
        ).dropna()
...
class UCF101_2STREAM_LMDB_2CLIP(object):
    def __init__(
        ...
        root="%s/../process_data/data/ucf101"
        % os.path.dirname(os.path.abspath(__file__)),
        db_path_flow=os.path.join(lmdb_root, "Flow/ucf101_tvl1_frame.lmdb"),
        db_path_rgb=os.path.join(lmdb_root, "RGB/ucf101_frame.lmdb"),
        transform=None,
        mode="val",
        num_frames=32,
        ds=1,
        ...
        print(
            "Loading flow LMDB from %s, split:%d"
            % (self.db_path_flow, self.which_split)
        )
        self.env_flow = lmdb.open(
            self.db_path_flow,
        ...
        print(
            "Loading rgb LMDB from %s, split:%d" % (self.db_path_rgb, self.which_split)
        )
        self.env_rgb = lmdb.open(
            self.db_path_rgb,
        ...
        vlen_list_ordered = sorted(
            list(zip([i.decode() for i in self.db_keys_flow], self.vlen_list_flow)),
            key=lambda x: x[0],
        )
        vlen_list_ordered = [i[-1] for i in vlen_list_ordered]

        vlen_df_flow = pd.DataFrame(
            zip(vname_list_flow, vlen_list_ordered), columns=[3, 4]
        )
        ...
        self.get_video_id_flow = dict(
            zip(
                [i.decode() for i in self.db_order_flow],
                ["%09d" % i for i in range(len(self.db_order_flow))],
            )
        )
        self.get_video_id_rgb = dict(
            zip(
                [i.decode() for i in self.db_order_rgb],
    ...
    def __getitem__(self, index):
        ...
        with env_rgb.begin(write=False) as txn:
            raw_rgb = msgpack.loads(
                txn.get(self.get_video_id_rgb[vname].encode("ascii")), raw=True
            )
        env_flow = self.env_flow
        with env_flow.begin(write=False) as txn:
            raw_flow = msgpack.loads(
                txn.get(self.get_video_id_flow[vname].encode("ascii")), raw=True
            )
        ...
        seq = self.transform(
            seq_rgb[0 : self.num_frames]
            + seq_flow[0 : self.num_frames]
            + seq_rgb[self.num_frames : :]
            + seq_flow[self.num_frames : :]
        )
...
class KineticsLMDB_2CLIP(object):
    def __init__(
        ...
        video_info = pd.read_csv(
            os.path.join(root, "%s_split.csv" % split_mode), header=None
        )
...
class K400_LMDB_2CLIP(KineticsLMDB_2CLIP):
    def __init__(
        self,
        root="%s/../process_data/data/k400"
        % os.path.dirname(os.path.abspath(__file__)),
        db_path=os.path.join("/users/htd/beegfs/", "kinetics400-256/frame_full/lmdb/"),
        filename="k400_frame",
        **kwargs
    ):
        super(K400_LMDB_2CLIP, self).__init__(
            root=root, db_path=db_path, filename=filename, is_flow=False, **kwargs
        )
...
class K400_Flow_LMDB_2CLIP(KineticsLMDB_2CLIP):
    def __init__(
        ...
        **kwargs
        ...
        super(K400_Flow_LMDB_2CLIP, self).__init__(
            root=root, db_path=db_path, filename=filename, is_flow=True, **kwargs
        )
...
class Kinetics_2STREAM_LMDB_2CLIP(object):
    def __init__(
        ...
        if video_info.iloc[0][3].split("/").__len__() != 2:  # long path to short path
            video_info[3] = (
                video_info[3].str.split("/").str.slice(-2, None, 1).str.join("/")
            )
...
class K400_2STREAM_LMDB_2CLIP(Kinetics_2STREAM_LMDB_2CLIP):
    def __init__(
        ...
        root="%s/../process_data/data/k400"
        % os.path.dirname(os.path.abspath(__file__)),
        db_path_flow=os.path.join(
            "/users/htd/beegfs/", "kinetics400-256/tvl1_flow/lmdb/"
        ),
        db_path_rgb=os.path.join(
            "/users/htd/beegfs/", "kinetics400-256/frame_full/lmdb/"
        ),
        filename_flow="k400_tvl1_frame",
        filename_rgb="k400_frame",
        **kwargs
    ):
        super(K400_2STREAM_LMDB_2CLIP, self).__init__(
            root=root,
            db_path_flow=db_path_flow,
            db_path_rgb=db_path_rgb,
            filename_flow=filename_flow,
            filename_rgb=filename_rgb,
            **kwargs
        )
