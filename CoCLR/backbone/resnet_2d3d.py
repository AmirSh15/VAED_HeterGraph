...
__all__ = ["ResNet2d3d", "r2d3d50", "r3d50"]
...
class Bottleneck3d(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, use_final_relu=True
    ):
        super(Bottleneck3d, self).__init__()
        bias = False
        self.use_final_relu = use_final_relu
        self.conv1 = nn.Conv3d(
            inplanes, planes, kernel_size=(3, 1, 1), padding=(1, 0, 0), bias=bias
        )
...
class ResNet2d3d(nn.Module):
    def __init__(self, block, layers, input_channel=3):
        ...
        self.layer4 = self._make_layer(
            block[3], 512, layers[3], stride=(1, 2, 2), is_final=True
        )
...
def r3d50(**kwargs):
    """Constructs a ResNet-50 model. """
    model = ResNet2d3d(
        [Bottleneck3d, Bottleneck3d, Bottleneck3d, Bottleneck3d], [3, 4, 6, 3], **kwargs
    )
    return model
