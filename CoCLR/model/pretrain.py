...
class InfoNCE(nn.Module):
    ...
    def forward(self, block):
        """Output_clip_len_1.0_audio_10: logits, targets"""
...
class UberNCE(InfoNCE):
    ...
    def forward(self, block, k_label):
        """Output_clip_len_1.0_audio_10: logits, binary mask for positive pairs
        """
        (B, N, *_) = block.shape  # [B,N,C,T,H,W]
        assert N == 2
        x1 = block[:, 0, :].contiguous()
...
class CoCLR(InfoNCE):
    ...
    def __init__(
        self, network="s3d", dim=128, K=2048, m=0.999, T=0.07, topk=5, reverse=False
    ):
        """
        dim: feature dimension (default: 128)
        K: queue size; number of negative keys (default: 2048)
    ...
    def forward(self, block1, block2, k_vsource):
        ...
        # dequeue and enqueue
        if in_train_mode:
            self._dequeue_and_enqueue(k, kf, k_vsource)
