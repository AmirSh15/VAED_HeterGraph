...
from MultiModalGraph.utils.utils import (
    AverageMeter,
    Logger,
    ProgressMeter,
    calc_topk_accuracy,
...
def parse_args():
    ...
    parser.add_argument(
        "--test",
        default="../../CoCLR-ucf101-rgb-128-s3d-ep182.tar",
        type=str,
        help="path of model to load and pause",
    )
    parser.add_argument(
        "--retrieval",
        default="/media/amir_shirian/Amir/Datasets/Sound Recognition/UCF101/RGB",
        action="store_true",
        help="path of model to ucf retrieval",
    )
...
def main(args):
    ...
    if args.test:
        ...
        transform_test_cuda = transforms.Compose(
            [
                T.Normalize(
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], channel=1
                )
            ]
        )
        ...
        if args.retrieval:
            ...
            test_10crop(
                test_dataset, model, ce_loss, transform_test_cuda, device, epoch, args
            )
        else:
            raise NotImplementedError
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_freq == 0:
            _, val_acc = validate(
                val_loader, model, ce_loss, transform_val_cuda, device, epoch, args
            )
...
def train_one_epoch(
    data_loader, model, criterion, optimizer, transforms_cuda, device, epoch, args
):
...
def validate(data_loader, model, criterion, transforms_cuda, device, epoch, args):
    ...
    print(
        "Epoch: [{0}]\t"
        "Loss: {loss.avg:.4f} Acc@1: {top1.avg:.4f} Acc@5: {top5.avg:.4f}\t".format(
            epoch, loss=losses, top1=top1_meter, top5=top5_meter
        )
    )
...
def test_10crop(dataset, model, criterion, transforms_cuda, device, epoch, args):
    ...
    acc_final = summarize_probability(
        prob_dict, data_loader.dataset.encode_action, "ten"
    )
...
def summarize_probability(prob_dict, action_to_idx, title):
    ...
    for vname, item in tqdm(prob_dict.items(), total=len(prob_dict)):
        ...
        mean_top1, mean_top5 = calc_topk_accuracy(
            mean_prob, torch.LongTensor([target]).cuda(), (1, 5)
        )
        stat[vname] = {"mean_prob": mean_prob.tolist()}
        acc[0].update(mean_top1.item(), 1)
        acc[1].update(mean_top5.item(), 1)

    print("Mean: Acc@1: {acc[0].avg:.4f} Acc@5: {acc[1].avg:.4f}".format(acc=acc))
...
def test_retrieval(model, criterion, transforms_cuda, device, epoch, args):
    ...
    def tr(x):
        ...
        return (
            transforms_cuda(x)
            .view(
                3, test_sample, args.num_seq, args.seq_len, args.img_dim, args.img_dim
            )
            .permute(1, 2, 0, 3, 4, 5)
        )
    ...
    with torch.no_grad():
        ...
        if os.path.exists(
            ...
            for idx, (input_seq, target) in tqdm(
                ...
                if test_feature is None:
                    test_feature = torch.zeros(
                        len(test_dataset), feature.size(-1), device=feature.device
                    )
        ...
        if os.path.exists(
            ...
            for idx, (input_seq, target) in tqdm(
                ...
                if train_feature is None:
                    train_feature = torch.zeros(
                        len(train_dataset), feature.size(-1), device=feature.device
                    )
        ...
        torch.save(
            sim,
            os.path.join(
                os.path.dirname(args.test), dirname, "%s_sim.pth.tar" % args.dataset
            ),
        )
        ...
        with open(
            os.path.join(
                os.path.dirname(args.test), dirname, "%s_test_vname.pkl" % args.dataset
            ),
            "rb",
        ) as fp:
        ...
        with open(
            os.path.join(
                os.path.dirname(args.test), dirname, "%s_train_vname.pkl" % args.dataset
            ),
            "rb",
        ) as fp:
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    ratio = 0.1 if epoch in args.schedule else 1.0
...
def get_data(transform, mode, args):
    ...
    if args.dataset == "ucf101":
        dataset = UCF101LMDB(
            mode=mode,
            transform=transform,
            num_frames=args.seq_len * args.num_seq,
            ds=args.ds,
