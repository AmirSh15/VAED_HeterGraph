...
class BasicConv3d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding=0):
        ...
        self.conv.weight.data.normal_(
            mean=0, std=0.01
        )  # original s3d is truncated normal within 2 std
...
class S3D(nn.Module):
    def __init__(self, input_channel=3, gating=False, slow=False):
        ...
        if slow:
            self.Conv_1a = STConv3d(
                input_channel, 64, kernel_size=7, stride=(1, 2, 2), padding=3
            )
        else:  # normal
            self.Conv_1a = STConv3d(
                input_channel, 64, kernel_size=7, stride=2, padding=3
            )
        ...
        self.Mixed_3b = SepInception(
            in_planes=192, out_planes=[64, 96, 128, 16, 32, 32], gating=gating
        )
        self.Mixed_3c = SepInception(
            in_planes=256, out_planes=[128, 128, 192, 32, 96, 64], gating=gating
        )
        ...
        self.Mixed_4b = SepInception(
            in_planes=480, out_planes=[192, 96, 208, 16, 48, 64], gating=gating
        )
        self.Mixed_4c = SepInception(
            in_planes=512, out_planes=[160, 112, 224, 24, 64, 64], gating=gating
        )
        self.Mixed_4d = SepInception(
            in_planes=512, out_planes=[128, 128, 256, 24, 64, 64], gating=gating
        )
        self.Mixed_4e = SepInception(
            in_planes=512, out_planes=[112, 144, 288, 32, 64, 64], gating=gating
        )
        self.Mixed_4f = SepInception(
            in_planes=528, out_planes=[256, 160, 320, 32, 128, 128], gating=gating
        )
        ...
        self.Mixed_5b = SepInception(
            in_planes=832, out_planes=[256, 160, 320, 32, 128, 128], gating=gating
        )
        self.Mixed_5c = SepInception(
            in_planes=832, out_planes=[384, 192, 384, 48, 128, 128], gating=gating
        )
    ...
    def forward(self, x):
        ...
        x = self.block4(x)
        x = self.block5(x)
        feat3d = F.adaptive_avg_pool3d(x, (1, 1, 1))  # [B,C,1,1,1]
        feat3d = feat3d.view(B, 1024)  # [B,C]
