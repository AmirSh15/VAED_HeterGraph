...
def main():
    """ Output_clip_len_1.0_audio_10 the top 5 Kinetics classes predicted by the model """
    path_sample = "./sample"
    ...
    list_frames = [
        f
        for f in os.listdir(path_sample)
        if os.path.isfile(os.path.join(path_sample, f))
    ]
    list_frames.sort()
